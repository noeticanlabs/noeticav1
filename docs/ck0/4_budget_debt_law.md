...
| **A5** | Lipschitz control | See domain specification below | Bounded sensitivity to debt |
| **A6** | Continuity | Continuous or piecewise continuous | No sudden jumps |

### A5 Enhanced: Lipschitz Domain Specification

The service law Φ : ℝ_{≥0} × ℝ_{≥0} → ℝ_{≥0} MUST satisfy ONE of:

**(a) Global Lipschitz on Domain:**
```
∃L ≥ 0 such that ∀D₁, D₂ ∈ [0, D_max], ∀B ∈ ℝ_{≥0}:
  |Φ(D₁, B) - Φ(D₂, B)| ≤ L·|D₁ - D₂|
```

**(b) Piecewise-Certified Lipschitz:**
```
∃ partition {R₁, ..., R_p} of [0, D_max]
∃ constants {L₁, ..., L_p} ≥ 0
∃ region selection rule ρ: ℝ_{≥0} → {1, ..., p}
such that ∀D₁, D₂ ∈ R_i:
  |Φ(D₁, B) - Φ(D₂, B)| ≤ L_i·|D₁ - D₂|
```

Where D_max is the declared maximum admissible debt (per abort rules).

**Rationale:** Without explicit domain specification, a service law could exhibit explosive curvature outside the tested range, making boundedness proofs vacuous.
...
| Property | Benefit |
|----------|---------|
| Audit clarity | "Decrease comes only from service; increase comes only by declared disturbance" |
...
## Disturbance Separation Rule (non-negotiable)

**Rule:** Φ (equivalently S) MUST NOT include disturbance. All exogenous effects MUST be accounted for solely in E_k.

This prevents double-counting and closes the "hidden slack" loophole.
...
## Disturbance Policy Classes (DP0-DP3)

Every CK-0 run MUST declare a `disturbance_policy_id` selecting one of the following policies:
...
There exists a declared $\bar{E} \ge 0$ such that:
...
There exists a finite event type set $\mathcal{T}$ and bound map:

$\beta : \mathcal{T} \to \mathbb{R}_{\ge 0}$

Each step declares an optional disturbance event ($t_k \in \mathcal{T} \cup {\varnothing}$), and:

$0 \le E_k \le \beta(t_k)$

with $\beta(\varnothing)=0$. Verifier checks event id + bound.
...
with fixed parameters $\theta$ and canonical arithmetic. Verifier recomputes E_k exactly. Use this only when you need "computed disturbance" (sensor noise models, discretization error estimators) without sacrificing replay.

---

### Receipt obligations for disturbance

Every step receipt MUST include:

| Field | Type | Description |
|-------|------|-------------|
| `disturbance_policy_id` | String | One of DP0, DP1, DP2, DP3 |
| `E_k` | DebtUnit | Disturbance value |
| `disturbance_event_id` | String | (DP2 only) Event type or null |
| `E_bar` | DebtUnit | (DP1 only) Global bound Ē |
| `E_model_id` | String | (DP3 only) Model identifier |
| `E_model_params_hash` | Hash | (DP3 only) Parameter commitment |
